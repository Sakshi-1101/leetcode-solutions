class Solution {
    ...
    // only difference is that we are using Character.isLetterOrDigit instead of checking both isDigit and isAlphabetic
    public boolean isPalindromeApproach2(String s) {
        ...
        while(i <= j){
            ...
            if(!Character.isLetterOrDigit(ch1)){
                i++;
            }else if(!Character.isLetterOrDigit(ch2)){ // move fwd if jth char is not alphanumeric
                j--;
            }else if (ch1 == ch2){
                i++;
                j--;
